...
describe("Step4 fixed-slot production contract", () => {
  ...
  test("passes explicit dynamic paragraph and slot counts without a misleading static example", () => {
    ...
    expect(prompt).not.toContain("把完整的 'to verb' 作为 answer");
    expect(prompt).not.toContain("仅在完整句其他位置出现知识点");
    expect(prompt).toContain("answer 可以按句法变化，也可以与 cue 相同");
    expect(prompt).toContain("grammarPoints 只定义一次");
