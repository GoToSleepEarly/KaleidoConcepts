...
export const STEP4_CONTENT_CONTRACT_VERSION = "step4.content.v11" as const;
...
function normalizeChapterSlotPlacement(value: unknown) {
  if (!value || typeof value !== "object") return value;
  const record = value as Record<string, unknown>;
  const rawParagraphs = record.paragraphs;
  if (!Array.isArray(rawParagraphs)) return value;
  const nestedSlots: unknown[] = [];
  const paragraphs = rawParagraphs.map((paragraph) => {
    ...
    if (Array.isArray(paragraphRecord.slots)) nestedSlots.push(...paragraphRecord.slots);
    return Object.fromEntries(Object.entries(paragraphRecord).filter(([key]) => key !== "slots"));
  });
  const rootSlots = Array.isArray(record.slots) ? record.slots : [];
  if (!rootSlots.length && !nestedSlots.length) return { ...record, paragraphs };
  const slots: unknown[] = [...rootSlots];
  const rootSlotById = new Map<string, unknown>();
  for (const slot of rootSlots) {
    const id = slot && typeof slot === "object" && typeof Reflect.get(slot, "id") === "string" ? Reflect.get(slot, "id") as string : null;
    if (id && !rootSlotById.has(id)) rootSlotById.set(id, slot);
  }
  for (const slot of nestedSlots) {
    const id = slot && typeof slot === "object" && typeof Reflect.get(slot, "id") === "string" ? Reflect.get(slot, "id") as string : null;
    const rootSlot = id ? rootSlotById.get(id) : undefined;
    if (rootSlot && JSON.stringify(rootSlot) === JSON.stringify(slot)) continue;
    slots.push(slot);
  }
  return { ...record, paragraphs, slots };
}

const strictGeneratedChapterTemplateSchema = z.object({
...
export const generatedChapterTemplateSchema = z.preprocess(normalizeChapterSlotPlacement, strictGeneratedChapterTemplateSchema);
const generatedChapterTemplateResponseSchema = z.preprocess(normalizeChapterSlotPlacement, z.object({
  outlineChapterId: requiredText,
...
const candidateChapterTemplateSchema = z.preprocess(normalizeChapterSlotPlacement, z.object({
  outlineChapterId: requiredText,
  paragraphs: z.array(z.object({ template: requiredText }).strip()).min(1),
...
export type ChapterGrammarPoint = {
  ...
  unitStart?: number;
  unitEnd?: number;
  sourceUnits?: Array<{ unitNumber: number; officialTitle: string; learningContents?: string[] }>;
...
function promptGrammarPoint({ key, label, category, unitStart, unitEnd, sourceUnits }: ChapterGrammarPoint) {
...
function promptChapterSpec(chapter: ReadingTemplatePromptContext["chapters"][number]) {
  ...
  return {
    ...
    paragraphCount: chapter.requirements.paragraphCount,
    grammarCount: chapter.requirements.grammarCount,
    vocabularyCount: chapter.requirements.vocabularyCount,
    totalSlotCount: chapter.requirements.grammarCount + chapter.requirements.vocabularyCount,
    ...
    grammarPoints: chapter.requirements.grammarPoints.map((point) => ({ key: point.key, definitionKey: point.definitionKey ?? point.key })),
    ...(chapter.knowledgePointUsagePlan ? { knowledgePointUsagePlan: chapter.knowledgePointUsagePlan } : {}),
  };
}

export const grammarQuestionSemanticsRule = "根级 grammarPoints 中的 label 是 Grammar in Use 的 Unit 名称，units[].learningContents 是该 Unit 的核心语法要点。每道语法题依据所绑定 Unit 的至少一项语法要点生成，整道题能有效考查该要点即可，不要求单题覆盖全部要点。同一 Unit 有多道题时优先考查不同要点或使用不同语境，但不强行凑齐。题型首先根据语法要点和当前语境选择；只有多种已启用题型同样合适时，才考虑总体均衡。";
const readingParagraphBalanceRule = "将 requiredSlotIds 覆盖到全部段落并尽可能均匀分布；题目数不少于段落数时，每段都安排题目，任意两段的题目数目标相差不超过 1。各段完整 clean text 的词数同样围绕 paragraphBudgets.preferredRange 尽可能均衡。";
const wordFormCueRule = "wordForm 的 cue 是横线后括号内展示的给词提示，必须是目标实词的词典原形，例如 answer='went' 时 cue='go'，answer='was waiting' 时 cue='wait'；answer 可以按句法变化，也可以与 cue 相同。answer 与紧邻语境构成的完整题目必须能练习绑定知识点。";

export function buildReadingTemplatePrompt(context: ReadingTemplatePromptContext) {
  const grammarPoints = [...new Map(context.chapters.flatMap((chapter) => chapter.requirements.grammarPoints).map((point) => [point.definitionKey ?? point.key, { ...point, key: point.definitionKey ?? point.key }])).values()].map(promptGrammarPoint);
  const payload = {
  ...
  return [
    ...
    `requiredSlotIds 每个都在 template 与 slots 中各出现一次；GR 槽位的 kind 只能来自 enabledGrammarTypes。knowledgePointKey 仅来自本章 grammarPoints[].key 且全部覆盖。${readingParagraphBalanceRule}${grammarQuestionSemanticsRule}`,
    ...
    "返回前先做两轮内部检查：第一轮回填全部答案并逐句通读纯正文；第二轮检查每个题目的 cue、distractors、知识点、决定性语境、章节短键、paragraphCount、grammarCount、vocabularyCount、totalSlotCount、固定槽位和词数。先修正全部错误，不输出检查过程。",
    "<context>",
...
export function buildReadingTemplateFinalizationPrompt(candidateOutput: unknown, context: ReadingTemplatePromptContext) {
  ...
  return [
    ...
    `第二步审核题目：候选位置、answer、cue 和 GR 槽位的 kind 均可修改，但 kind 只能来自该章 enabledGrammarTypes。${readingParagraphBalanceRule}${grammarQuestionSemanticsRule}${wordFormCueRule}optionCloze 返回两个标准、完整且拼写正确的 distractors；wordForm 即使候选缺少 cue，也必须在最终 slots 中补齐正确 cue。逐项回填后只有 answer 能同时满足当前语法、时间线和语义，否则改写局部上下文提供决定性线索。vocabulary 即使候选缺少 canonicalForm 或 meaningZh，也必须在最终 slots 中补齐，且三个字段一致、answer 不含连字符。`,
...
export function buildReadingTemplateRepairPrompt(targets: Array<{
  ...
  const adjacentSummary = (chapter: ReadingTemplatePromptContext["chapters"][number] | undefined) => chapter
    ...
    : undefined;
  const structuralIssueCodes = new Set<ChapterTemplateIssueCode>(["paragraph_count", "slot_set", "marker_set", "part_structure"]);
  const repairMode = (target: (typeof targets)[number]) => !target.current || target.issues.some((issue) => structuralIssueCodes.has(issue.code)) ? "chapter" : "paragraph";
  const grammarPoints = [...new Map(targets.flatMap((target) => target.requirements.grammarPoints).map((point) => [point.definitionKey ?? point.key, { ...point, key: point.definitionKey ?? point.key }])).values()].map(promptGrammarPoint);
  return [
    "一次修复全部失败目标，只返回严格 JSON：{contractVersion,repairs:[...],mainIdea?}。不得返回或修改未列出的成功章节；课后阅读未失败时不得返回 mainIdea。",
    "严格遵守每个 target 的 repairMode。repairMode='paragraph' 时返回 paragraph repair={kind:'paragraph',outlineChapterId,paragraphIndex,template,slots:[只含该段需新增或修改的槽位]}。repairMode='chapter' 时恰好返回一个 chapter repair={kind:'chapter',outlineChapterId,chapter:{outlineChapterId,paragraphs:[{template}],slots:[slot]}}；paragraphs 恰好为 spec.paragraphCount 项且段落对象内不得放 slots，全部题槽统一放在 chapter.slots，数量恰好为 spec.totalSlotCount。outlineChapterId 只能原样复制目标中的 C1/C2 等章节短键。",
    ...
    "英语正确性是最高优先级：修复后的答案回填句必须在语法、时态、主谓一致、单复数、代词、助动词、介词、语序和时间逻辑上正确。分别回填每个 distractor，只要任一项在当前语法、时间线和语义中也成立，就先改写局部上下文使 answer 成为唯一正确答案。不得为了题量、知识点覆盖、字数或故事表达保留错误英语。",
    ...
    `只修目标问题，不得借修复增加冲突、反转、支线或改变事实。knowledgePointKey 只能来自 spec.grammarPoints[].key，全部知识点至少覆盖一次；requiredSlotIds 必须在模板和 slots 中各恰好出现一次。${readingParagraphBalanceRule}${grammarQuestionSemanticsRule}${wordFormCueRule}`,
    ...
    JSON.stringify({ contractVersion: STEP4_CONTENT_CONTRACT_VERSION, mainIdeaTarget, mainIdeaPolicy: mainIdeaTarget ? context.mainIdea : undefined, targets: targets.map((target) => {
      ...
      return {
        ...
        spec: chapter ? promptChapterSpec(chapter) : {
          ...
          paragraphCount: target.requirements.paragraphCount,
          grammarCount: target.requirements.grammarCount,
          vocabularyCount: target.requirements.vocabularyCount,
          totalSlotCount: target.requirements.grammarCount + target.requirements.vocabularyCount,
          ...
          grammarPoints: target.requirements.grammarPoints.map((point) => ({ key: point.key, definitionKey: point.definitionKey ?? point.key })),
        },
