...
describe("course content repository", () => {
  ...
  test("generates chapters and Main Idea together, then uses the one shared repair budget for an invalid Main Idea", async () => {
    ...
    const repairReading = vi.fn(async () => ({ contractVersion: "step4.content.v6" as const, repairs: [], mainIdea: { text: Array(120).fill("summary").join(" ") } }));
  ...
  test("repairs one failed fixed-slot chapter once without replacing a successful chapter", async () => {
    ...
    const repairReading = vi.fn(async () => ({ contractVersion: "step4.content.v6" as const, repairs: [{ kind: "paragraph" as const, outlineChapterId: "chapter-2", paragraphIndex: 0, template: `${words} {{GR1}}`, slots: [] }], usage: { inputTokens: 40, outputTokens: 20, visibleOutputTokens: 15, reasoningTokens: 5, totalTokens: 60 } }));
