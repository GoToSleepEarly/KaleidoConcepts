...
describe("account AI gateway route", () => {
  ...
  test("GET reads the current gateway from the authenticated user's database row", async () => {
    findUnique.mockResolvedValue({ id: "user-1", aiGateway: "crazyrouter", quickRouterEndpoint: "direct" });
    const response = await GET(new Request("http://localhost/api/account/ai-gateway", {
      headers: { cookie: "kaleido.user-id=user-1" },
    }));

    await expect(response.json()).resolves.toEqual({ aiGateway: "crazyrouter", quickRouterEndpoint: "direct" });
  ...
  test("PATCH updates the database without storing the gateway in a cookie", async () => {
    findUnique.mockResolvedValue({ id: "user-1", aiGateway: "quickrouter", quickRouterEndpoint: "main" });
    update.mockResolvedValue({ id: "user-1", aiGateway: "crazyrouter", quickRouterEndpoint: "direct" });
    ...
    await expect(response.json()).resolves.toEqual({ aiGateway: "crazyrouter", quickRouterEndpoint: "direct" });
    expect(update).toHaveBeenCalledWith({ where: { id: "user-1" }, data: { aiGateway: "crazyrouter", quickRouterEndpoint: "direct" } });
    expect(response.headers.get("set-cookie")).toBeNull();
  });

  test("PATCH preserves the endpoint for a cached client that only sends the gateway", async () => {
    findUnique.mockResolvedValue({ id: "user-1", aiGateway: "quickrouter", quickRouterEndpoint: "direct" });
    update.mockResolvedValue({ id: "user-1", aiGateway: "crazyrouter", quickRouterEndpoint: "direct" });
