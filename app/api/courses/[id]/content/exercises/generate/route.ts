...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (regenerate) {
      ...
      if (hasDownstream && !resetDownstream) return NextResponse.json({ message: "重新生成成功后将重置视觉资源和预览发布", requiresReset: true }, { status: 409 });
      if (hasDownstream) {
        await generateCourseExercises(db, id, key, createCourseContentGenerationDeps(aiGateway), { regenerate, writingProvider: aiGateway.writingProvider });
        await clearCourseAfterStage(downstreamDb, id, "content", "content");
        return NextResponse.json(await getCourseContentState(db, id));
      }
    }
    return NextResponse.json(await generateCourseExercises(db, id, key, createCourseContentGenerationDeps(aiGateway), { regenerate, writingProvider: aiGateway.writingProvider }));
  }
  catch (error) {
