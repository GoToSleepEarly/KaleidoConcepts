...
import { generateVisualSlot, getCourseVisualResources } from "@/lib/server/repositories/visual-resources";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const targets = state.slots.filter((slot) => shouldGenerateVisualSlot(slot, input));
    if (targets.length === 0) {
      ...
      if (requestedSlots.some((slot) => hasInFlightVisualVersion(slot.versions))) return NextResponse.json({ message: "图片正在生成，请勿重复提交" }, { status: 409 });
