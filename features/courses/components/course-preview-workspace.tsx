...
type StoredPdfExport = Pick<PdfExportProgress, "completedPages" | "totalPages"> & { startedAt: number };
...
export function CoursePreviewWorkspace({ initialState }: { initialState: CoursePreviewResponse }) {
  ...
  useEffect(() => {
    const beforeUnload = (event: BeforeUnloadEvent) => { if (hasChanges || pending === "pdf") event.preventDefault(); };
  ...
  useEffect(() => {
    ...
    try {
      ...
      return () => window.clearTimeout(timer);
    } catch { window.sessionStorage.removeItem(pdfStorageKey); }
  }, [pdfStorageKey]);
  ...
  async function downloadPdf() {
    ...
    setPending("pdf"); setError(null); setPdfNotice(null); setPdfOutcome(null); setPdfCancelling(false); setPdfProgress(null); setInterruptedPdf(null); setPdfElapsedSeconds(0);
    try {
      const result = await exportSlidesToPDF(".preview-deck-pdf", `${initialState.course.title.replace(/[\\/:*?"<>|]/g, "_")}.pdf`, {
        signal: controller.signal,
        onProgress: (progress) => {
          setPdfProgress(progress);
          window.sessionStorage.setItem(pdfStorageKey, JSON.stringify({ completedPages: progress.completedPages, totalPages: progress.totalPages, startedAt: pdfStartedAt.current } satisfies StoredPdfExport));
        },
      });
      ...
      else { setPdfProgress(null); setError(reason instanceof Error ? reason.message : "PDF 下载失败"); }
    } finally {
  ...
  return <main className="mx-auto max-w-6xl space-y-6">
    ...
    <div className="flex flex-wrap items-start justify-between gap-4"><div><div className="mb-4 flex flex-wrap gap-2"><Button disabled={pending === "pdf"} onClick={() => void navigate(`/courses/${initialState.course.id}/create/visual-resources`)} size="sm" variant="outline"><ArrowLeft />返回视觉资源</Button><Button disabled={pending === "pdf"} onClick={() => void navigate("/courses")} size="sm" variant="ghost"><LayoutList />返回课程列表</Button></div><h1 className="text-xl font-semibold">课程预览与发布</h1><p className="mt-2 text-sm text-muted-foreground">检查课件页面和练习题型，确认后发布进入授课模式。</p></div><div className="flex flex-wrap items-center gap-2"><span aria-live="polite" className={cn("text-xs font-medium", displaySaveStatus === "failed" ? "text-destructive" : displaySaveStatus === "saving" ? "text-primary" : "text-muted-foreground")}>{displaySaveStatus === "dirty" ? "待自动保存" : displaySaveStatus === "saving" ? "正在自动保存…" : displaySaveStatus === "failed" ? "自动保存失败" : "已自动保存"}</span><div className="inline-flex overflow-hidden rounded-lg border text-sm"><button aria-label="课件预览" className={cn("min-h-10 px-4 py-2 transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50", mode === "html" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted")} disabled={pending === "pdf"} onClick={() => setMode("html")} type="button">课件预览</button><button aria-label="打印预览" className={cn("min-h-10 px-4 py-2 transition active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50", mode === "pdf" ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground hover:bg-muted")} disabled={pending === "pdf"} onClick={() => setMode("pdf")} type="button">打印预览</button></div>{mode === "pdf" ? <Button disabled={pending === "pdf"} onClick={downloadPdf} size="sm" variant="outline"><Download />{pending === "pdf" ? "正在导出" : "下载 PDF"}</Button> : null}<Button disabled={pending === "pdf" || (!hasChanges && displaySaveStatus !== "failed")} loading={displaySaveStatus === "saving"} onClick={() => void save(presentation)} size="sm" variant="outline"><Save />{displaySaveStatus === "failed" ? "重试保存" : "保存草稿"}</Button><Button disabled={pending === "pdf"} onClick={() => setShowPublishConfirm(true)} size="sm"><Send />发布课程</Button></div></div>
    ...
    {pdfProgress && (pending === "pdf" || pdfOutcome) ? <aside aria-label="PDF 导出进度" aria-live="polite" className={cn("animate-fade-in fixed bottom-5 right-5 z-50 w-[min(22rem,calc(100vw-2rem))] rounded-xl border bg-card p-4 shadow-lg", pdfOutcome === "success" && "border-success/40", pdfOutcome === "cancelled" && "border-border")} role="status"><div className="flex items-start justify-between gap-4"><div><p className="font-semibold">{pdfOutcome === "success" ? "导出完成" : pdfOutcome === "cancelled" ? "已取消导出" : "正在导出 PDF"}</p><p className="mt-1 text-sm text-muted-foreground">{pdfOutcome ? pdfNotice : pdfCancelling ? "正在取消，完成当前页后停止" : pdfProgressLabel(pdfProgress)}</p></div>{pdfOutcome ? <Button aria-label="关闭 PDF 导出结果" className="-mr-2 -mt-2" onClick={closePdfResult} size="sm" variant="ghost">关闭</Button> : <span className="shrink-0 text-xs tabular-nums text-muted-foreground">{pdfElapsedSeconds} 秒</span>}</div><div aria-label="PDF 导出进度" aria-valuemax={100} aria-valuemin={0} aria-valuenow={Math.round((pdfProgress.completedPages / Math.max(1, pdfProgress.totalPages)) * 100)} className="mt-3 h-2 overflow-hidden rounded-full bg-muted" role="progressbar"><div className={cn("h-full rounded-full bg-primary", pdfOutcome === "success" && "bg-success")} style={{ width: `${(pdfProgress.completedPages / Math.max(1, pdfProgress.totalPages)) * 100}%` }} /></div>{!pdfOutcome ? <div className="mt-3 flex items-center justify-between gap-3"><p className="text-xs text-muted-foreground">请保持此页面打开；刷新会中断导出。</p>{pdfProgress.phase === "preparing" || pdfProgress.phase === "rendering" ? <Button aria-label="取消导出" disabled={pdfCancelling} onClick={() => { setPdfCancelling(true); pdfAbortController.current?.abort(); }} size="sm" variant="outline">{pdfCancelling ? "取消中" : "取消"}</Button> : null}</div> : null}</aside> : null}
  </main>;
}
