...
function ImageLayer({ url, dim = false, eager = false }: { url: string | null; dim?: boolean; eager?: boolean }) {
  return url ? <><Image alt="课件插图" className="object-cover" fill loading={eager ? "eager" : "lazy"} sizes="(max-width: 1200px) 80vw, 900px" src={url} unoptimized />{dim ? <span className="absolute inset-0 bg-slate-950/35" /> : null}</> : <div className="absolute inset-0 flex items-center justify-center bg-slate-200 text-sm text-slate-500">图片尚未采用</div>;
}
...
export function CourseSlideDeck({ pages, presentation, showAllPages = false, pdfBackgroundMode = "image", selectedPageId, onSelectPage }: { pages: CoursePreviewPage[]; presentation: CoursePresentationConfig; showAllPages?: boolean; pdfBackgroundMode?: PreviewSlideBackgroundMode; selectedPageId?: string; onSelectPage?: (id: string) => void }) {
  ...
  if (showAllPages) return <div className="preview-deck-pdf flex flex-col gap-4">{pages.map((page) => <div className="preview-slide-wrapper aspect-video overflow-hidden rounded-lg shadow" data-page-type={page.type} key={page.id}><PreviewSlide backgroundMode={pdfBackgroundMode} mode="pdf" page={page} presentation={presentation} /></div>)}</div>;
