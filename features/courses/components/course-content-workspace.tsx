...
export function CourseContentWorkspace({ initialState }: { initialState: CourseContentState }) {
  ...
  async function updateProvider(writingProvider: StoryWritingProvider) {
    ...
    const response = await fetch(`/api/courses/${state.course.id}/content/settings`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ writingProvider }),
    });
        ...
        <aside className={cn("min-w-0", sections.length && "lg:h-full lg:min-h-0 lg:overflow-hidden")}>
          <section className={cn("flex min-w-0 flex-col overflow-hidden rounded-lg bg-card shadow-sm", sections.length ? "h-[calc(100dvh-18rem)] min-h-[480px] max-h-[760px] lg:h-full lg:max-h-none lg:min-h-0 lg:overflow-hidden" : "min-h-[clamp(560px,calc(100dvh-18rem),720px)]", sections.length && mobileView === "preview" && "hidden lg:flex")} data-testid="content-chat-pane">
            <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
              ...
              <select aria-label="文本生成模型" className="h-9 rounded-md border border-input bg-muted/60 px-2 text-xs font-medium text-foreground" disabled={isWorking} onChange={(event) => updateProvider(event.target.value as StoryWritingProvider)} value={state.writingProvider}>
                <option value="quickrouter_gpt">GPT</option>
                <option value="quickrouter_deepseek">DeepSeek</option>
              </select>
            </div>
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain scroll-pb-24 p-4" data-testid="content-chat-scroll" ref={chatScrollRef}>
              ...
              {state.messages.map((message, index) =>
                isRepairMessage(message.content) ? (
                  <AssistantMessage key={message.id}>
                    <RepairMessage failed={state.status === "failed" && index === latestRepairMessageIndex} message={message.content} working={repairInProgress && index === latestRepairMessageIndex} />
                  </AssistantMessage>
                ) : (
                  <ContentChatMessage key={message.id} role={message.role === "teacher" ? "teacher" : "assistant"} system={message.role === "system"}>
                    {message.content}
                  </ContentChatMessage>
                ),
              )}
              ...
              {!isWorking && state.status === "failed" ? (
                <ChatAction title={(error || state.errorMessage)?.includes("超时") ? "本次等待已超时；重试会发起新请求，已保存内容不会重复生成" : "上次生成未完成，已成功内容不会重复生成"}>
                  <Button className="w-full" onClick={() => generate("reading")}>
                    <Sparkles className="size-4" />
                    {mainIdeaFailed ? "重试课后阅读" : "重试未通过内容"}
                  </Button>
                </ChatAction>
              ) : null}
              {!isWorking && !state.chapters.length && state.status !== "failed" ? (
                <ChatAction title="生成后可逐页检查和修改">
                  <Button onClick={() => generate("reading")} size="sm">
                    <Sparkles className="size-4" />
                    生成正文与课后阅读
                  </Button>
                </ChatAction>
              ) : null}
              {!isWorking && state.status === "reading_ready" ? (
                <ChatAction title="正文与课后阅读已生成">
                  <div className="space-y-2">
                    <Button className="w-full" onClick={() => generate("exercises")}>
                      确认正文与课后阅读，生成练习
                      <ChevronRight className="size-4" />
                    </Button>
                    <Button className="w-full" onClick={() => generate("reading", true)} variant="outline">
                      <RotateCcw className="size-4" />
                      重新生成正文与课后阅读
                    </Button>
                  </div>
...
function RepairMessage({ message, working, failed }: { message: string; working: boolean; failed: boolean }) {
  ...
  return (
    <details className={cn("group w-fit max-w-xl rounded-lg border text-sm", status === "working" ? "border-amber-200 bg-amber-50 text-amber-950" : status === "failed" ? "border-destructive/30 bg-destructive/5 text-destructive" : "border-emerald-200 bg-emerald-50 text-emerald-950")} key={status} open={status !== "completed" || undefined}>
