...
2026-09-23：修复自定义图片传输层把全局 `FormData` 交给外部 `undici.fetch` 后被序列化成 `text/plain` 的回归。图片请求继续使用同源的 `globalThis.fetch` 与全局 `FormData`，仅注入自定义 Undici dispatcher；新增课程多参考图和人物单参考图的真实 multipart 集成测试，直接校验 boundary、model、prompt 与图片字段。
