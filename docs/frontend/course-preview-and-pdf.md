...
- 课程发布API
- 课件slide共用渲染组件
- 预览发布前校验没有图片任务仍在生成；没有资源方案或图片时允许显示占位
- 创建流Step5中的课件编辑功能：
  - 封面标题样式调整（字体大小、蒙版主题）
  - 章节分隔页配色选择（预设主题）
  - 文本框样式/位置调整
...
恢复边界：
- 无课文草稿：回Step3生成或编辑草稿。
- 图片生成中：阻断进入预览与发布，留在 Step4 等待；直达 URL 与 API 同样不能绕过。图片缺失、未采用或失败时允许使用占位继续。
...
PDF导出属于Step5核心交付能力，不能只按桌面Chrome验证。任何修改Step5预览、slide组件、PDF导出工具、全局样式、图片加载策略、字体缩放或主题变量时，必须同时考虑并验证以下端：

- **PC端**：桌面 Chrome / Edge / Safari 至少覆盖一个 Chromium 浏览器；导出 PDF 必须完整分页、图片清晰、文本不溢出、不显示答案和操作区。
- **iPad端**：iPadOS Safari 是必须考虑的目标；导出流程不得依赖桌面专属下载行为、超大canvas、hover交互或仅鼠标可用控件。
- **移动端**：手机浏览器可进入打印预览并触发导出；如浏览器因系统限制不能直接保存文件，必须给出明确的用户可恢复路径（例如打开PDF预览、新标签查看或提示使用系统分享/保存）。

跨端导出必须满足：
- PDF内容尺寸以固定16:9 slide逻辑尺寸为真相源，不得依赖当前屏幕上的CSS像素宽度；手机、iPad和PC导出的版式应一致。
- 导出DOM必须与可交互编辑DOM解耦，避免移动端响应式布局、滚动容器、缩放、sticky/fixed控制栏影响截图范围。
- 单页canvas尺寸和总导出内存必须有上限，避免iOS/iPadOS Safari因canvas面积或内存限制导出失败、空白或崩溃。
- 图片导出前必须完成加载或走明确占位；跨域、懒加载、object-fit、透明蒙版在各端导出结果必须一致。
- 不得使用 `hover`、键盘快捷键、桌面文件下载能力作为唯一操作路径；触屏端按钮尺寸和状态反馈必须可用。
- 导出失败必须显示可理解错误信息，并保留用户回到预览、重试或改用浏览器打印/分享的恢复路径。
...
- PDF导出兼容性覆盖 PC、iPad 和手机视口，验证固定16:9版式、分页、图片、文本、隐藏答案和导出失败提示。
- PDF导出测试不得只断言函数调用成功；必须检查导出前DOM尺寸、canvas尺寸上限、移动端触发路径和不兼容CSS颜色函数处理。
...
- PDF导出必须在 PC、iPad 和手机端具备明确可用路径；同一课程在不同端导出的slide顺序、16:9版式、答案隐藏规则和图片占位规则必须一致。
- 移动端或iPad端如受浏览器限制无法直接下载文件，页面必须提供明确提示和可恢复替代路径，不能静默失败或长时间卡在加载状态。
...
- 2026-07-31：优化 PDF 下载跨端稳定性：
  - **固定导出尺寸**：PDF下载不再截图当前响应式预览尺寸，改为导出期间创建离屏 `1600x900` 固定16:9 slide DOM，确保 PC、iPad、手机导出版式一致。
  - **canvas上限**：导出scale按单页canvas像素预算自动收敛，降低 iOS/iPadOS Safari 因canvas面积或内存限制导致空白、失败或崩溃的风险。
  - **DOM隔离**：导出DOM在截图后立即移除，避免移动端滚动容器、响应式宽度、编辑页布局和控制栏影响导出范围。
  - 验证：`pnpm vitest run lib/utils/pdf-export.test.ts`（7 passed）；`pnpm lint` 通过。
