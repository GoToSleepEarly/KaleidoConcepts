...
- 将课程status从draft改为published。
- 返回发布后的授课页URL。
- 发布后课程配置不可再通过编辑API修改（如需修改必须先回退到草稿状态，MVP暂不做回退，后续迭代加）。

失败：
- `400 { message: "请先生成课文草稿" }`
- `400 { message: "请先生成所有必需图片" }`（可选：MVP允许部分图片缺失时也能发布，缺图显示占位符）
- `404 { message: "课程不存在" }`
- `409 { message: "课程已发布" }`
