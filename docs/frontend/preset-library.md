...
- 表单里手输的自定义主题 / 语法点**不自动入库**（用户确认），只对当前课程生效，仍走 `Course.theme` / `Course.grammar` 字符串存储
- 编辑已有课程时，若课程的 theme / grammar 不在预设库里（历史值或已删除的预设），仍显示为已选中项，逻辑同现有 `themeOptions` / `grammarOptions` 的合并策略
