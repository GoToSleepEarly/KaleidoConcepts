...
- 状态：已实现，待用户验收。
- 本文档是语法知识点来源、Unit 粒度、只读知识库、课程选择器和下游语法上下文的唯一契约。
...
本期不复制教材正文、讲解、例句或练习。知识库保存可核验的书目与目录信息，并为每个 Unit 维护产品侧撰写的 3–4 条核心语法要点；正式商用导入完整目录前仍需完成版权与授权审查。
...
一个 Grammar in Use Unit 就是一个可独立选择的系统知识点。每个知识点拥有稳定 `KnowledgePoint.id`，并只关联一个准确来源 Unit。

任何 `1 / 2 / 3 ...` 连续编号 Unit 都不得合并。编号表示教材将相近主题拆成了不同教学范围；标题、选择、语法要点、AI 上下文和覆盖校验均保持独立。

示例：

```text
知识点一：Unit 13 · Present perfect and past 1
知识点二：Unit 14 · Present perfect and past 2
```
...
- 覆盖校验以独立 Unit 知识点为单位。
- 搜索同时匹配 Unit 编号、完整官方标题和语法要点。
- 学生内容不展示书籍来源和 Unit 编号。
...
教师工作区展示书名、版本、官方等级、Section 官方英文标题、Unit 官方标题和 3–4 条核心语法要点。`learningContents` 是产品依据 Unit 教学范围撰写的规则摘要，不宣称为 Cambridge 官方原文；每条必须包含明确语法形式、固定搭配、使用条件或易混区别，不允许“正确使用”“在自然语境中使用”等空泛内容。
...
- `/grammar` 知识库每行以 Unit 编号和官方英文标题为主信息，直接完整展示全部 3–4 条语法要点，不能截取第一条作为不完整摘要。
- Step 1–3 均保留各模块原有标签、选择行和勾选卡样式，不使用新的统一摘要外观。每个原有知识点项只在右侧增加无文字下拉箭头，点击后在当前项下方完整展示 3–4 条语法要点。
- 同一列表默认全部收起，同时只展开一个 Unit；查看详情、选择、勾选和移除必须使用独立点击区域。
...
- 手机：全屏单栏，书籍与 Section 使用顶部选择控件，语法要点随 Unit 完整展示。
...
`GrammarUnitProfile` 是随代码版本管理的产品语义层，目录 API 与 AI 上下文在读取时合并；不复制到课程 JSON，也不新增数据库字段。
...
- `grammar_in_use` 知识点必须关联书籍版本和 Section，且恰好关联一个来源 Unit。
- 同一书籍版本内 Unit 编号唯一；知识点 ID 固定为 `<bookEditionId>-u<unitNumber>`。
...
Step 4 为降低重复输入，同一任务只传一次书名、版本和官方难度；每个 Unit 的名称和 3–4 条核心语法要点在任务根级只定义一次，各章节或练习目标仅引用短键。语法要点不包含例题、反例或题型绑定。

生成、修复和老师局部修改共用同一语义：每道题依据绑定 Unit 的至少一项语法要点生成，整道题能有效考查该要点即可；不要求单题覆盖全部要点。同一 Unit 有多道题时优先考查不同要点或语境，但不强行凑齐；先按语法要点和语境选择题型，只有多种已启用题型同样合适时才考虑总体均衡。
...
- 每个系统知识点只对应一个准确 Unit；连续编号的 `1 / 2 / 3` Unit 也分别产生独立可选 ID。
...
- 2026-09-12 Unit 粒度重构：取消连续编号标题合并，三册共 365 个 Unit 均成为独立知识点；每个 Unit 预置 3–4 条核心语法要点，并通过测试禁止空泛表述、数量越界和同册完全重复范围。知识库与选择器完整展示语法要点，流程摘要按需展开但不截断；Step 4 生成、修复和局部修改统一按 Unit 语法要点出题。Seed 在新目录成功写入后清理旧的合并知识点。验证通过 93 个文件 / 807 项测试、`pnpm lint`、`pnpm exec tsc --noEmit`、`pnpm build`、`git diff --check`，并完成 1240px 桌面与 390 × 844 手机真实浏览器检查。提交号：本轮未提交。
- 内置数据：三册共 365 个 Unit、52 个 Section，编译为 365 个独立系统知识点；导入时校验册内 Unit 数量、Unit 独立性和每个 Unit 的 3–4 条语法要点。
...
- 2026-09-13 Unit 详情交互：Step 1–3 恢复各自原有视觉样式，仅增加右侧无文字下拉箭头和原位语法要点展开；查看、选择、勾选与移除互不影响。Step 2 AI 仍仅在完整大纲生成和章节知识点推荐修改时接收所选 Unit 的 3–4 条语法要点，其他对齐阶段不增加上下文。提交号：本轮未提交。
- 2026-09-14 Step 2–3 验收：章节推荐、本章知识点、选择器和课后知识点均完成真实页面交互检查；全量 94 个文件 / 811 项测试与生产构建通过，浏览器控制台无错误，乱码扫描和 `git diff --check` 通过。提交号：本轮未提交。
- 2026-09-14 长标题适配：紧凑选择器的浏览行与“已选”行都限制标题占用可伸缩区域；已选摘要保持单行，Unit、详情箭头和移除操作不随标题长度移动。Step 3 课后考查知识点与正文知识点统一为同一组胶囊布局规则。全量 94 个文件 / 812 项测试与生产构建通过。提交号：本轮未提交。
